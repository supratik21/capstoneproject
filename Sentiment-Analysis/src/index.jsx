...
const updateIssuePriority = async (issueId, priorityId) => {
...
const App = () => {
  ...
  const handlePriorityChange = async () => {
    const priorityId = priorityMap[image];
    if (priorityId) {
      const response = await api.asApp().requestJira(`/rest/api/3/issue/${context.platformContext.issueKey}`, {
        method: 'PUT',
        headers: {
          'Accept': 'application/json',
          'Content-Type': 'application/json'
  ...
  const priorityMap = {
    "Positive": "1",          // Highest
    "Slightly Positive": "2", // High
    "Neutral": "4",           // Low
    "Slightly Negative": "3", // Medium
    "Negative": "2"           // High
  };
  ...
  if(count > 6){
    ...
    image = "Slightly Positive"
  }else if(count >= -2) {
    image = "Neutral"
  }
  else if(count >= -6) {
    image = "Slightly Negative"
  }else{
    image = "Negative"
  }
  if(rows.length > 0){
    ...
    return (
      <Fragment>
        <Text>{image}</Text>
        <Button text={`Re-assign Priority`} onClick={handlePriorityChange} />
        <Button text={`View Breakdown`} onClick={() => setOpen(true)} />
        {isOpen && (
